...
let threeNumSum = require('../Arrays/threeNumSum')

describe('Array Problems', function() {
  describe('threeNumSum', function() {

    it('should return an empty array when no three integers equal the target', function() {
      assert.typeOf(threeNumSum([1,3,5,8,9], 99), 'array')
      assert.deepEqual(threeNumSum([1,3,5,8,9], 99), [])
    });

    it('should return an empty array when not enough integers are provided', function() {
      assert.deepEqual(threeNumSum([5,9], 14), [])
    });

    it('should return triplet sums with digits in ascending order', function() {
      assert.deepEqual(threeNumSum([1,2,3], 6), [[1,2,3]])
      assert.deepEqual(threeNumSum([4,6,2], 12), [[2,4,6]])
    });

    it('should return the right triplet that sums to target', function() {
      assert.deepEqual(threeNumSum([8,3,17,6,2], 17), [[3,6,8]])
    });

    it('should return triplet sums in ascending order', function() {
      assert.deepEqual(threeNumSum([1,2,4,6,12,13], 19), [[1,6,12],[2,4,13]])
    });
  });
